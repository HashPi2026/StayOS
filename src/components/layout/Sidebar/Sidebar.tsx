...
interface NavItem {
  ...
  label: string;
  icon: string;
  subItems?: { id: NavigationPath; label: string; icon: string }[];
}
    ...
    <aside className="fixed left-0 top-0 h-full w-[240px] bg-[#f2f4f6] z-50 flex flex-col border-r border-[#c6c6cd]/60 select-none">
      ...
      <div className="px-4 py-6 flex flex-col gap-4">
        <div 
          ...
          title="Return to Main PMS Dashboard"
        >
        ...
        <button
          id="exit-configuration-btn"
          onClick={() => navigate('dashboard')}
          className="flex items-center justify-center gap-2 w-full py-2.5 bg-[#0f172a] text-white rounded-lg text-label-uppercase hover:bg-[#1e293b] active:scale-[0.98] transition-all shadow-sm cursor-pointer"
          title="Redirect to Main PMS Screen (11 Modules)"
        >
              ...
              {section.items.map((item) => {
                ...
                return (
                  <div key={item.id} className="space-y-0.5">
                    <div
                      className={`w-full flex items-center justify-between px-3 py-2 rounded-md transition-all text-left text-[13px] group cursor-pointer ${
                        active && !hasSubItems
                    ...
                    >
                      <div className="flex items-center min-w-0">
                        <span
                          className={`material-symbols-outlined mr-2.5 text-[19px] shrink-0 ${
                            active && !hasSubItems
                              ? 'text-white'
                              : active && hasSubItems
                              ? 'text-[#0058be]'
                        ...
                        {item.subItems!.map((sub) => {
                          ...
                          return (
                            <button
                              ...
                              className={`w-full flex items-center px-2.5 py-1.5 rounded-md transition-all text-left text-[12px] ${
                                isSubActive
                            ...
                            >
                              <span
                                className={`material-symbols-outlined mr-2 text-[15px] shrink-0 ${
                                  isSubActive ? 'text-white' : 'text-[#75859d]'
                                }`}
                              >
                                {sub.icon}
                              </span>

...
export const getCountryEmoji = (countryOrCode: string): string => {
...
export const CountryFlag: React.FC<CountryFlagProps> = ({
  ...
  if (flagUrl) {
    return (
      <div
        className={`${className} rounded-sm bg-cover bg-center border border-[#c6c6cd]/40 shrink-0`}
  ...
  // Find in presets
  const matched = COUNTRY_PRESETS.find(
    (c) => c.country.toLowerCase() === country.toLowerCase() || (countryCode && c.code.toLowerCase() === countryCode.toLowerCase())
  );
  ...
  if (matched?.flagUrl) {
    return (
      <div
        className={`${className} rounded-sm bg-cover bg-center border border-[#c6c6cd]/40 shrink-0`}
        style={{ backgroundImage: `url('${matched.flagUrl}')` }}
      />
    );
  }
  ...
  return (
    <div className={`${className} flex items-center justify-center text-sm leading-none bg-[#eceef0] rounded-sm shrink-0 select-none`}>
      <span>{emoji}</span>
    </div>
  );
};

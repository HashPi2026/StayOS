...
import { RoomStatus } from '@/src/types';

export const EditRoomView: React.FC = () => {
  const {
    rooms,
    selectedRoomId,
    buildings,
  ...
  // Initialize from targetRoom
  useEffect(() => {
    if (targetRoom) {
      setRoomName(targetRoom.name || `Room ${targetRoom.number}`);
      setShortName(targetRoom.shortName || targetRoom.number);
      setBuildingId(targetRoom.buildingId);
      setStatus(targetRoom.status);
      setRoomTypeId(targetRoom.roomTypeId);

      // Find matching floor
      const matchedFloor = floors.find(
        (f) =>
          f.buildingId === targetRoom.buildingId &&
          (f.id === targetRoom.floorId ||
            f.floorNumber === targetRoom.floor ||
            f.name.toLowerCase().includes(String(targetRoom.floor)))
      );
      setFloorId(matchedFloor ? matchedFloor.id : (targetRoom.floorId || ''));

      setIsHourlyRental(!!targetRoom.isHourlyRental);
      setIsSmoking(!!targetRoom.isSmoking);
      setIsHandicapAccessible(!!targetRoom.isHandicapAccessible);
      setIsPetAllowed(!!targetRoom.isPetAllowed);
      setIncludeInOccupancyAdr(targetRoom.includeInOccupancyAdr !== undefined ? targetRoom.includeInOccupancyAdr : true);
      setIsCrsInventory(targetRoom.isCrsInventory !== undefined ? targetRoom.isCrsInventory : true);
  ...
  const handleRoomNameChange = (val: string) => {
    ...
    } else if (!isRoomNameUnique(val, targetRoom?.id)) {
  ...
  const handleShortNameChange = (val: string) => {
    ...
    } else if (!isRoomShortNameUnique(val, targetRoom?.id)) {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (!roomName.trim()) {
      setErrorRoomName('Room name is required');
      hasErrors = true;
    } else if (!isRoomNameUnique(roomName, targetRoom.id)) {
      setErrorRoomName('A room with this name already exists in the property');
      hasErrors = true;
    }
    ...
    if (!shortName.trim()) {
      setErrorShortName('Short name is required');
      hasErrors = true;
    } else if (!isRoomShortNameUnique(shortName, targetRoom.id)) {
      setErrorShortName('A room with this short name already exists');
      hasErrors = true;
    }
    ...
    const success = updateRoom(targetRoom.id, {
      name: roomName.trim(),
      shortName: shortName.trim(),
      number: shortName.trim(),
      buildingId,
      buildingName: bld?.name || targetRoom.buildingName,
      floorId,
      floor: numericFloor,
      floorName: flr?.name || `Floor ${numericFloor}`,
      roomTypeId,
      roomTypeName: rt?.name || targetRoom.roomTypeName,
      rate: rt?.baseRate || targetRoom.rate,
      status,
      isHourlyRental,
      isSmoking,
  ...
  if (!targetRoom) {
    return (
      <div className="p-8 text-center">
        <p className="text-[14px] text-[#45464d]">No room selected.</p>
        <button
          onClick={() => navigate('rooms')}
          className="mt-4 px-4 py-2 bg-[#000000] text-white rounded-lg text-[13px] font-semibold"
        >
          Return to Rooms
        </button>
      </div>
  ...
  return (
    <div className="flex flex-col w-full h-full min-h-[calc(100vh-4rem)] p-6 lg:p-8 space-y-8 bg-[#f7f9fb]" id="edit-room-view">
      {/* Page Header */}
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-2 max-w-4xl mx-auto w-full">
        <div className="space-y-1">
          <nav className="flex items-center text-[13px] text-[#45464d]">
            ...
            <span className="material-symbols-outlined text-[16px] mx-1">chevron_right</span>
            <span className="text-[#191c1e] font-semibold">Edit {targetRoom.name || `Room ${targetRoom.number}`}</span>
          </nav>
          <h1 className="text-[30px] font-bold text-[#191c1e] tracking-tight">Edit Room</h1>
        </div>
        <button
          onClick={() => navigate('rooms')}
      ...
      <form onSubmit={handleSubmit} className="flex-1 max-w-4xl mx-auto w-full space-y-6 pb-32">
        ...
        <section className="bg-white rounded-xl p-6 md:p-8 shadow-xs border border-[#c6c6cd]/30 space-y-4 relative overflow-hidden">
          ...
          <div className="flex items-start justify-between pb-3 border-b border-[#e0e3e5] relative z-10">
            <div>
              <h2 className="text-[18px] font-semibold text-[#191c1e]">Hierarchy & Operational Status</h2>
              <p className="text-[13px] text-[#45464d] mt-1">Configure location and PMS occupancy status.</p>
            </div>
            <span className="material-symbols-outlined text-[#45464d] p-2 bg-[#eceef0] rounded-full text-[20px]">
              account_tree
            </span>
          ...
          <div className="flex items-center gap-2 py-2 overflow-x-auto text-[13px] font-mono text-[#45464d] relative z-10">
            ...
            <span className="px-2.5 py-1 bg-[#000000] text-white rounded font-medium shadow-2xs">
              {shortName.trim() ? `Room ${shortName.trim()}` : targetRoom.name}
            </span>
          </div>
          ...
          <div className="pt-2 relative z-10">
            ...
            <div className="flex flex-wrap gap-2">
              {(['available', 'occupied', 'reserved', 'cleaning', 'maintenance'] as RoomStatus[]).map((st) => (
                <button
                  key={st}
                  type="button"
                  onClick={() => setStatus(st)}
                  className={`px-3.5 py-1.5 rounded-lg text-[13px] font-semibold capitalize border transition-all ${
                    status === st

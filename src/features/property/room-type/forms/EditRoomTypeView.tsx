...
import { RoomCategory, RoomTypeStatus, RoomType } from '@/src/types';
...
export const EditRoomTypeView: React.FC = () => {
  const {
    selectedRoomTypeId,
    roomTypes,
    navigate,
  ...
  const currentRoomType = roomTypes.find((rt) => rt.id === selectedRoomTypeId) || roomTypes[0];

  // State initialized with current room type
  ...
  // Sync state when currentRoomType changes
  useEffect(() => {
    if (currentRoomType) {
      setName(currentRoomType.name || '');
      setCode(currentRoomType.code || '');
      setCategory(currentRoomType.category || 'Deluxe');
      setStatus(currentRoomType.status || 'active');
      setDescription(currentRoomType.description || '');

      setCapacity(currentRoomType.capacity || 2);
      setMaxAdults(currentRoomType.maxAdults || currentRoomType.capacity || 2);
      setMaxChildren(currentRoomType.maxChildren || 1);
      setSizeSqm(currentRoomType.sizeSqm || 38);
      setSizeSqft(currentRoomType.sizeSqft || 409);
      setViewType(currentRoomType.viewType || 'City Skyline View');
      setSmokingPolicy(currentRoomType.smokingPolicy || 'non-smoking');
      setIsAccessible(currentRoomType.isAccessible || false);

      setBedType(currentRoomType.bedType || '1 King Bed');
      setBedCount(currentRoomType.bedCount || 1);
      setExtraBedAllowed(currentRoomType.extraBedAllowed || false);
      setMaxExtraBeds(currentRoomType.maxExtraBeds || 1);

      setBaseRate(currentRoomType.baseRate || 280);
      setExtraAdultRate(currentRoomType.extraAdultRate || 50);
      setExtraChildRate(currentRoomType.extraChildRate || 25);

      setTotalUnits(currentRoomType.totalUnits || 0);
      setSelectedBuildings(currentRoomType.buildingIds || (buildings.length > 0 ? [buildings[0].id] : []));
      setSelectedAmenities(currentRoomType.amenities || ['High-speed Wi-Fi 6', 'Rain Shower']);
      setImageUrl(currentRoomType.imageUrl || PRESET_IMAGES[0].url);
      setTouched({});
    }
  }, [currentRoomType, buildings]);
  ...
  const isNameDuplicate =
    name.trim().length > 0 && !isRoomTypeNameUnique(name, currentRoomType?.id);
  const isCodeEmpty = code.trim().length === 0;
  const isCodeDuplicate =
    code.trim().length > 0 && !isRoomTypeCodeUnique(code, currentRoomType?.id);
  ...
  const handleSave = () => {
    ...
    if (hasFormErrors || !currentRoomType) return;

    const success = updateRoomType(currentRoomType.id, {
      name: name.trim(),
  ...
  if (!currentRoomType) {
    return (
      <div className="flex flex-col items-center justify-center p-12 text-center">
        <h2 className="text-title-lg text-[#191c1e] font-semibold">Room Type not found</h2>
        <button
          onClick={() => navigate('room-types')}
          className="mt-4 px-4 py-2 bg-[#0058be] text-[#ffffff] rounded-lg"
        >
          Return to Room Types
        </button>
      </div>
    );
  }
  ...
  return (
    <div className="flex flex-col w-full h-full max-w-[1280px] mx-auto px-4 sm:px-6 py-6 min-h-screen bg-[#f7f9fb]">
      ...
      <div className="sticky top-16 bg-[#f7f9fb]/95 backdrop-blur-md z-20 py-3 border-b border-[#c6c6cd]/30 flex flex-col md:flex-row md:items-center justify-between gap-4 mb-6">
        <div>
          <nav className="flex items-center text-body-sm text-[#75859d] mb-1">
            ...
            <span className="material-symbols-outlined text-[16px] mx-1">chevron_right</span>
            <span className="text-[#191c1e] font-semibold">Edit {currentRoomType.name}</span>
          </nav>
            ...
            <div>
              <div className="flex items-center gap-2">
                <h1 className="font-semibold text-headline-md text-[#191c1e] tracking-tight">
                  Edit {currentRoomType.name}
                </h1>
                <span className="font-data-mono text-[12px] bg-[#dae2fd] text-[#131b2e] px-2 py-0.5 rounded font-semibold">
                  {currentRoomType.code}
                </span>
              </div>
              <p className="font-body-md text-[#45464d] text-sm mt-0.5">
                Update specifications, occupancy limitations, tariffs, and building inventory allocations.
              </p>
            </div>
          </div>
        </div>
        ...
        <div className="flex items-center gap-3">
          <button
            onClick={() => duplicateRoomType(currentRoomType.id)}
            className="px-3.5 py-2 border border-[#c6c6cd] rounded-lg text-body-md font-medium text-[#191c1e] hover:bg-[#eceef0] transition-colors cursor-pointer flex items-center gap-1.5"
            title="Create a copy of this room type"
          >
            <span className="material-symbols-outlined text-[18px]">content_copy</span>
            Duplicate
          </button>
          <button
            onClick={() => openDeleteRoomTypeDialog(currentRoomType)}
            className="px-3.5 py-2 border border-[#ba1a1a]/40 text-[#ba1a1a] rounded-lg text-body-md font-medium hover:bg-[#ba1a1a]/10 transition-colors cursor-pointer flex items-center gap-1.5"
          >
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 items-start">
            ...
            <div className="space-y-4">
              ...
              <div>
                ...
                <textarea
                  ...
                  placeholder="Detail the room atmosphere, premium features, view orientation..."
        ...
        {/* Right Sidebar: 4 cols - Live Preview */}

...
export class RoomStatusRepository {
  async findMany(clientId: string, isActive?: boolean): Promise<RoomStatusEntity[]> {
    let text = `
      SELECT
        room_status_id,
        client_id,
    ...
    const res = await query<RoomStatusEntity>(text, params);
    return res.rows;
  }

  async findById(clientId: string, statusId: number): Promise<RoomStatusEntity | null> {
    const text = `
      SELECT
        room_status_id,
        client_id,
      ...
      LIMIT 1;
    `;
    const res = await query<RoomStatusEntity>(text, [clientId, statusId]);
    return res.rows[0] || null;
  }

  async findByCode(clientId: string, statusCode: string): Promise<RoomStatusEntity | null> {
    const text = `
      SELECT
        room_status_id,
        client_id,
      ...
      LIMIT 1;
    `;
    const res = await query<RoomStatusEntity>(text, [clientId, statusCode.trim()]);
    return res.rows[0] || null;
  }

  async create(clientId: string, data: CreateRoomStatusDTO): Promise<RoomStatusEntity> {
    const text = `
    ...
    const res = await query<RoomStatusEntity>(text, values);
    return res.rows[0];
  }

  async update(clientId: string, statusId: number, data: UpdateRoomStatusDTO): Promise<RoomStatusEntity | null> {
    const text = `
      UPDATE room_status
      SET
  ...
  async softDelete(clientId: string, statusId: number): Promise<boolean> {
    const text = `
      UPDATE room_status
      SET is_active = FALSE
      WHERE client_id = $1 AND room_status_id = $2;

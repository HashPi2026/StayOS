...
export class RoomRepository {
  async findMany(
    ...
    const res = await query<RoomEntity>(text, params);
    return res.rows;
  }

  async findById(clientId: string, roomId: number): Promise<RoomEntity | null> {
    const text = `
      SELECT
        room_id,
        client_id,
      ...
      LIMIT 1;
    `;
    const res = await query<RoomEntity>(text, [clientId, roomId]);
    return res.rows[0] || null;
  }

  async create(clientId: string, data: CreateRoomDTO): Promise<RoomEntity> {
    const text = `
      INSERT INTO room (
        client_id,
        room_type_id,
    ...
    const res = await query<RoomEntity>(text, values);
    return res.rows[0];
  }

  async update(clientId: string, roomId: number, data: UpdateRoomDTO): Promise<RoomEntity | null> {
    const text = `
      UPDATE room
      SET
  ...
  async delete(clientId: string, roomId: number): Promise<boolean> {
    const text = `
      DELETE FROM room
      WHERE client_id = $1 AND room_id = $2;
    `;

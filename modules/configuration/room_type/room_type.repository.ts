...
export class RoomTypeRepository {
  async findMany(clientId: string): Promise<RoomTypeEntity[]> {
    const text = `
      SELECT
        room_type_id,
        client_id,
    ...
    `;
    const res = await query<RoomTypeEntity>(text, [clientId]);
    return res.rows;
  }

  async findById(clientId: string, roomTypeId: number): Promise<RoomTypeEntity | null> {
    const text = `
      SELECT
        room_type_id,
        client_id,
      ...
      LIMIT 1;
    `;
    const res = await query<RoomTypeEntity>(text, [clientId, roomTypeId]);
    return res.rows[0] || null;
  }

  async create(clientId: string, data: CreateRoomTypeDTO): Promise<RoomTypeEntity> {
    const text = `
    ...
    const res = await query<RoomTypeEntity>(text, values);
    return res.rows[0];
  }

  async update(clientId: string, roomTypeId: number, data: UpdateRoomTypeDTO): Promise<RoomTypeEntity | null> {
    const text = `
      UPDATE room_type
      SET
  ...
  async delete(clientId: string, roomTypeId: number): Promise<boolean> {
    const text = `

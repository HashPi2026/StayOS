...
import { taxService, TaxService } from './tax.service';
...
export class TaxController {
  ...
  private parseId(paramId: string, paramName = 'ID'): number {
  ...
  listTaxes = async (req: Request, res: Response, next: NextFunction): Promise<void> => {
    try {
  ...
  listConfigurations = async (req: Request, res: Response, next: NextFunction): Promise<void> => {
    try {
      const clientId = req.clientId!;
  ...
  getConfigurationById = async (req: Request, res: Response, next: NextFunction): Promise<void> => {
    try {
      const clientId = req.clientId!;
      const taxId = this.parseId(req.params.taxId, 'tax ID');
  ...
  createConfiguration = async (req: Request, res: Response, next: NextFunction): Promise<void> => {
    try {
      const clientId = req.clientId!;
      const taxId = this.parseId(req.params.taxId, 'tax ID');
  ...
  deleteConfiguration = async (req: Request, res: Response, next: NextFunction): Promise<void> => {
    try {
      const clientId = req.clientId!;
      const taxId = this.parseId(req.params.taxId, 'tax ID');
      const configId = this.parseId(req.params.configId, 'tax configuration ID');

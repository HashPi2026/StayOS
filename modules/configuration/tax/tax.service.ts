import { taxRepository, TaxRepository } from './tax.repository';
import {
  CreateTaxConfigDTO,
  CreateTaxDTO,
...
export class TaxService {
  constructor(private repo: TaxRepository = taxRepository) {}

  // ================= TAX =================

  async listTaxes(clientId: string, isActive?: boolean): Promise<TaxEntity[]> {
    return this.repo.findManyTaxes(clientId, isActive);
  }

  async getTaxById(clientId: string, taxId: number, includeConfigs = true): Promise<TaxEntity> {
  ...
  async createTax(clientId: string, dto: CreateTaxDTO): Promise<TaxEntity> {
    if (dto.per_day_tax === dto.per_stay_tax) {
      throw new ValidationError(
        "Tax basis check failed: exactly one of 'per_day_tax' and 'per_stay_tax' must be true (they cannot both be true or both be false).",
        { per_day_tax: dto.per_day_tax, per_stay_tax: dto.per_stay_tax }
      );
    }
    return this.repo.createTax(clientId, dto);
  }

  async updateTax(clientId: string, taxId: number, dto: UpdateTaxDTO): Promise<TaxEntity> {
    ...
    if (effectivePerDay === effectivePerStay) {
      throw new ValidationError(
        "Tax basis check failed: exactly one of 'per_day_tax' and 'per_stay_tax' must be true.",
        { per_day_tax: effectivePerDay, per_stay_tax: effectivePerStay }
      );
    }
  ...
  async deleteTax(clientId: string, taxId: number): Promise<void> {
    const existing = await this.repo.findTaxById(clientId, taxId);
    if (!existing) {
      throw new NotFoundError('Tax', taxId);
    }
   ...
   */
  private async validateNoActiveDateOverlap(
    clientId: string,
    taxId: number,
    fromDate: string,
    lastDate: string,
    ...
    for (const config of activeConfigs) {
      ...
      if (fromDate <= config.last_date && config.from_date <= lastDate) {
        throw new ValidationError(
          `Overlapping active date range detected. The date range '${fromDate}' to '${lastDate}' overlaps with existing active tax configuration (ID: ${config.tax_config_id}, '${config.from_date}' to '${config.last_date}').`,
          {
            tax_id: taxId,
  ...
  async updateConfiguration(
    ...
    if (effectiveIsActive) {
      await this.validateNoActiveDateOverlap(
        clientId,
        taxId,
        effectiveFromDate,
        effectiveLastDate,
  ...
  async deleteConfiguration(clientId: string, taxId: number, configId: number): Promise<void> {
    const existing = await this.repo.findConfigurationById(clientId, taxId, configId);
    if (!existing) {
      throw new NotFoundError('TaxConfiguration', configId);
    }

...
const isSupabaseOrRemote =
  Boolean(connectionString && (connectionString.includes('supabase.co') || connectionString.includes('pooler.supabase.com'))) ||
...
export async function testConnection(): Promise<{ connected: boolean; message: string; timestamp?: string }> {
  try {

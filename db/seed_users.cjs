...
async function seedUsers() {
  ...
  try {
    ...
    // 2. Roles for DIS_001
    const roles = [
      { id: 1, name: 'Property Administrator', short: 'Admin', type: 'ADMIN', desc: 'Full administrative access to Destin Inn & Suites PMS' },
      { id: 2, name: 'Front Desk Associate', short: 'FrontDesk', type: 'STAFF', desc: 'Front desk and guest check-in operations for Destin Inn & Suites' },
      { id: 5, name: 'General Manager', short: 'GM', type: 'ADMIN', desc: 'Executive hotel operations, approvals, and performance oversight' },
      { id: 6, name: 'Housekeeping Supervisor', short: 'HK-Sup', type: 'OPERATIONS', desc: 'Room cleaning inspection, attendant task assignments, and linen inventory' },
      { id: 7, name: 'Night Auditor', short: 'NightAudit', type: 'STAFF', desc: 'End-of-day closing, rate audits, and night front desk service' },
      { id: 8, name: 'Finance Controller', short: 'Finance', type: 'FINANCE', desc: 'Ledger balancing, folio taxation, invoices, and accounting reconciliation' },
      { id: 9, name: 'Front Desk Manager', short: 'FDM', type: 'STAFF', desc: 'Daily shift supervision, check-in flow, and room allocation' }
    ];

    for (const r of roles) {
      await client.query(`
        INSERT INTO role_privilege (role_id, client_id, role_name, short_name, role_type, description)
        VALUES ($1, 'DIS_001', $2, $3, $4, $5)
        ON CONFLICT (role_id) DO UPDATE SET
          role_name = EXCLUDED.role_name,
          short_name = EXCLUDED.short_name,
          role_type = EXCLUDED.role_type,
          description = EXCLUDED.description;
      `, [r.id, r.name, r.short, r.type, r.desc]);
    }
    ...
    for (const u of users) {
      await client.query(`
        ...
        VALUES ($1, 'DIS_001', $2, $3, $4, $5, $6, $7, $8, $9)
      ...
      `, [u.id, u.roleId, u.name, u.desc, u.active, u.phone, u.dept, u.avatar, u.initials]);

      await client.query(`
        INSERT INTO user_login_credential (credential_id, user_id, username, email, login_email, password_hash, is_active)
        VALUES ($1, $2, $3, $4, $4, '$2b$10$stayos_mock_hash_destin_2026', $5)
        ON CONFLICT (user_id) DO UPDATE SET
          username = EXCLUDED.username,
          email = EXCLUDED.email,
          login_email = EXCLUDED.login_email,
          is_active = EXCLUDED.is_active;
      `, [u.id, u.id, u.username, u.email, u.active]);
    }
    ...
    console.log('Successfully seeded 9 users for DIS_001 in PostgreSQL database!');
  } catch (err) {
